...
function writeCSV(filename, rows, columns) {
  ...
  fs.writeFileSync(path.join(DATA_DIR, filename), [header, ...lines].join('\n') + '\n', 'utf8');
...
async function collectProjects(context) {
  ...
  try {
    ...
    const cards = await page.locator('[data-testid="project-card"], article[class*="project"], li[class*="project"]').all();
    ...
    for (const card of cards) {
      ...
      projects.push({ title: title.trim(), description: desc?.trim() || null, creator: anonymise(creator?.trim()), updated_date: parseRelative(updated?.trim()), category: null, keywords: null, summary: null });
    }
    return projects;
...
async function collectConnectors(context) {
  ...
  try {
    ...
    const discoverTab = page.getByRole('tab', { name: /discover/i }).or(page.getByRole('button', { name: /discover/i }));
    ...
    await page.waitForLoadState('networkidle', { timeout: 15000 });
    const sortBtn = page.getByRole('button', { name: /sort|most used/i }).or(page.locator('select[name*="sort"], [class*="sort"]'));
    if (await sortBtn.count() > 0) {
      await sortBtn.first().click({ timeout: 5000 });
      const mostUsed = page.getByRole('option', { name: /most used/i }).or(page.getByText(/most used/i));
      if (await mostUsed.count() > 0) { await mostUsed.first().click({ timeout: 5000 }); await page.waitForLoadState('networkidle', { timeout: 10000 }); }
    }
    let prevCount = 0;
    for (let i = 0; i < 30; i++) {
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    ...
    for (const item of items) {
      ...
      const usageText = await item.locator('[class*="usage"], [class*="user"], [class*="count"]').first().textContent({ timeout: 3000 }).catch(() => null);
      const countMatch = usageText?.match(/(\d+)\s+user/i);
      const categoryText = await item.locator('[class*="categ"], [class*="tag"], [class*="badge"]').first().textContent({ timeout: 3000 }).catch(() => null);
      if (!name?.trim()) continue;
      connectors.push({ connector_name: name.trim(), provider: provider?.trim() || null, description: desc?.trim() || null, category: categoryText?.trim() || null, user_count: countMatch ? parseInt(countMatch[1], 10) : 0, snapshot_date: today() });
    }
    return connectors;
  } finally { await page.close(); }
}
...
async function collectPageViews(context) {
  ...
  for (const article of existing.articles) {
    ...
    try {
      ...
      const viewText = await page.locator('[class*="view-count"], [class*="article-view"], span:has-text("Views"), .kb-article-views').first().textContent({ timeout: 8000 }).catch(() => null);
      const viewMatch = viewText?.replace(/,/g, '').match(/(\d+)/);
      const views = viewMatch ? parseInt(viewMatch[1], 10) : null;
      if (views !== null) {
        if (!(article.history || []).some(h => h.date === date)) {
        ...
        } else { console.log(`    ${article.article_number}: already recorded for ${date}`); }
      } else { console.warn(`    ${article.article_number}: could not read view count — check selector`); }
    } finally { await page.close(); }
  }
  return existing;
}
...
async function main() {
  ...
  if (contexts.length === 0) throw new Error('No browser context found. Make sure Chrome is running with --remote-debugging-port=9222 and logged into Claude Enterprise.');
...
main().catch(err => { console.error('[collect] Fatal error:', err.message); process.exit(1); });
